...
def create_article_feature(
    ...
    with timer(f"Load article data: ({split})"):
        df = pl.concat(
            [
                pl.scan_parquet(
    ...
    return ArticleFeature(
        **{col.name: df[col.name].to_numpy() for col in fields(ArticleFeature)}
    )
...
def create_user_feature(
    ...
    with timer(f"Load user data ({split})"):
        df = pl.concat(
            [
                pl.scan_parquet(
                    PREPROCESSED_DIR / "v0200_users" / split / "dataset.parquet",
                    n_rows=n_rows,
                ).select(
                    pl.col("user_index"),
...
def create_impression_feature(
    ...
    with timer(f"Load impression data ({split})"):
        n_rows = 1_000_000 if debug else None
        df = pl.concat(
            [
                pl.scan_parquet(
                    PREPROCESSED_DIR / "v0300_impressions" / split / "dataset.parquet",
                    n_rows=n_rows,
                ).select(
    ...
    return ImpressionFeature(
        **{col.name: df[col.name].to_numpy() for col in fields(ImpressionFeature)}
    )
...
class LabelFeatureExtractor(FeatureExtractor):
    ...
    @property
    def fields(self) -> list[FeatureField]:
        return [LabelField("labels", collate_fn=CollateSequences(pad_value=-1))]
...
class ImpressionFeatureExtractor(FeatureExtractor):
    ...
    def __call__(
        ...
        outputs["inview_elapsed_min"] = (
            (outputs["imp_ts_inview"] - TRAIN_START_UNIXTIME) % SPLIT_INTERVAL // 60
        )
        return outputs
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.articles = {
            split: create_article_feature(split=split, debug=debug)
            for split in self.fold_splits_mapping[fold][split]
        }
        self.users = {
            split: create_user_feature(split=split, debug=debug)
            for split in self.fold_splits_mapping[fold][split]
        }
        self.impressions = {
            split: create_impression_feature(split=split, debug=debug, small=small)
            for split in self.fold_splits_mapping[fold][split]
        }
...
class Model(L.LightningModule):
    ...
    def forward(self, batch):
        ...
        h = self.encoder(inputs_embeds=xs["h"], attention_mask=xs["attention_mask"])
...
class MetricsMeter(torch.nn.Module):
    ...
    def update(self, batch: dict, n_samples: int | None = None):
        metrics = {}
        mask = batch["labels"] != -1
        _preds, _labels = batch["preds"][:n_samples], batch["labels"][:n_samples]
        ...
        if "bce_loss" in self.loss_weights:
            ...
            metrics["bce_loss"] = bce_losses.sum() / mask.sum()

        # Dice Loss
        if "dice_loss" in self.loss_weights:
            dice_losses = self.dice_loss(batch["preds"], batch["labels"].float())
            dice_losses = dice_losses.masked_fill(~mask, 0)
            metrics["dice_loss"] = dice_losses.sum() / mask.sum()
