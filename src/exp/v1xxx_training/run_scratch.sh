...
for MODEL in ${MODELS[@]}; do
    ...
    uv run python $MODEL predict --split validation common.fold=0 trainer.devices=[${DEVICE}]
    uv run python $MODEL predict --split test common.fold=2 trainer.devices=[${DEVICE}]
done