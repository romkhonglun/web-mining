...
from exputils.const import PREPROCESS_DIR, SPLIT_INTERVAL
...
def compute_features(
    ...
    min_ts = int(
        lf_impressions.select("impression_ts").min().collect().to_numpy().squeeze()
    )
    lf_impressions = (
        ...
        .with_columns(
            ...
            ((pl.col("impression_ts") - min_ts) % SPLIT_INTERVAL // 60).alias(
                "inview_elapsed_mins"
            ),
        )
