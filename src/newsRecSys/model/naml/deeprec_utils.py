...
# Giả định module này vẫn tồn tại trong project của bạn
from dataset.download_utils import maybe_download


def to_numpy(x):
    """
    Hàm tiện ích giúp chuyển đổi input thành numpy array.
    Hỗ trợ cả PyTorch Tensor (CPU/GPU), List, và Numpy array.
    """
...
def check_type(config):
    ...
    int_parameters = [
        ...
        "FEATURE_COUNT", "FIELD_COUNT", "dim", "epochs", "batch_size",
        "show_step", "save_epoch", "PAIR_NUM", "DNN_FIELD_NUM",
        "attention_layer_sizes", "n_user", "n_item", "n_user_attr",
        "n_item_attr", "item_embedding_dim", "cate_embedding_dim",
        "user_embedding_dim", "max_seq_length", "hidden_size",
        "T", "L", "n_v", "n_h", "kernel_size", "min_seq_length",
        "attention_size", "epochs", "batch_size", "show_step",
        "save_epoch", "train_num_ngs",
    ]
    ...
    float_parameters = [
        "init_value", "learning_rate", "embed_l2", "embed_l1",
        "layer_l2", "layer_l1", "mu",
    ]
    ...
    str_parameters = [
        "train_file", "eval_file", "test_file", "infer_file",
        "method", "load_model_name", "infer_model_name", "loss",
        "optimizer", "init_method", "attention_activation",
...
def check_nn_config(f_config):
    ...
    if f_config["model_type"] in ["fm", "FM"]:
        ...
        required_parameters = [
            "doc_size", "history_size", "wordEmb_file", "entityEmb_file",
            "contextEmb_file", "news_feature_file", "user_history_file",
            "word_size", "entity_size", "use_entity", "use_context",
            "data_format", "dim", "layer_sizes", "activation",
            "attention_activation", "attention_dropout", "loss",
            "dropout", "method", "num_filters", "filter_sizes",
        ]
    elif f_config["model_type"] in ["exDeepFM", "xDeepFM"]:
        required_parameters = [
            "FIELD_COUNT", "FEATURE_COUNT", "method", "dim",
            "layer_sizes", "cross_layer_sizes", "activation",
            "loss", "data_format", "dropout",
        ]
    elif f_config["model_type"] in ["gru4rec", "GRU4REC", "GRU4Rec"]:
        required_parameters = [
            "item_embedding_dim", "cate_embedding_dim", "max_seq_length",
            "loss", "method", "user_vocab", "item_vocab",
            "cate_vocab", "hidden_size",
        ]
    elif f_config["model_type"] in ["caser", "CASER", "Caser"]:
        required_parameters = [
            "item_embedding_dim", "cate_embedding_dim", "user_embedding_dim",
            "max_seq_length", "loss", "method", "user_vocab",
            "item_vocab", "cate_vocab", "T", "L", "n_v", "n_h",
            "min_seq_length",
        ]
    elif f_config["model_type"] in ["asvd", "ASVD", "a2svd", "A2SVD"]:
        required_parameters = [
            "item_embedding_dim", "cate_embedding_dim", "max_seq_length",
            "loss", "method", "user_vocab", "item_vocab", "cate_vocab",
        ]
    elif f_config["model_type"] in ["slirec", "sli_rec", "SLI_REC", "Sli_rec"]:
        required_parameters = [
            "item_embedding_dim", "cate_embedding_dim", "max_seq_length",
            "loss", "method", "user_vocab", "item_vocab", "cate_vocab",
            "attention_size", "hidden_size", "att_fcn_layer_sizes",
        ]
    elif f_config["model_type"] in ["nextitnet", "next_it_net", "NextItNet", "NEXT_IT_NET"]:
        required_parameters = [
            "item_embedding_dim", "cate_embedding_dim", "user_embedding_dim",
            "max_seq_length", "loss", "method", "user_vocab",
            "item_vocab", "cate_vocab", "dilations", "kernel_size",
            "min_seq_length",
        ]
    else:
        required_parameters = []
    ...
    if f_config["model_type"] in ["exDeepFM", "xDeepFM"]:
        if f_config["data_format"] != "ffm":
            raise ValueError(f"For xDeepFM model, data format must be 'ffm', but your set is {f_config['data_format']}")
    elif f_config["model_type"] in ["dkn", "DKN"]:
        if f_config["data_format"] != "dkn":
            raise ValueError(f"For dkn model, data format must be 'dkn', but your set is {f_config['data_format']}")
    check_type(f_config)
    ...
    def __init__(self, hparams_dict):
        for val in hparams_dict.values():
            if not (
                    ...
                    or val is None  # Cho phép None trong PyTorch config
            ):
                raise ValueError(
                    "Hyperparameter value {} should be integer, float, string or list.".format(val)
                )
...
def create_hparams(flags):
    ...
    init_dict = {
        ...
        "save_model": True,
        "save_epoch": 5,
        "write_tfevents": False,  # Có thể giữ lại nếu dùng tensorboardX hoặc torch.utils.tensorboard
        # sequential
        "train_num_ngs": 4,
        "need_sample": True,
...
def ndcg_score(y_true, y_score, k=10):
    ...
    # Lưu ý: dcg_score đã xử lý to_numpy bên trong
...
def cal_metric(labels, preds, metrics):
    """
    Calculate metrics.
    Updated to handle both Numpy arrays and PyTorch Tensors.
    """
    res = {}

    # Chuyển đổi sang numpy ngay từ đầu để dùng cho tất cả các metrics
    ...
    for metric in metrics:
        if metric == "auc":
            ...
            # avoid logloss nan
            preds_safe = [max(min(p, 1.0 - 10e-12), 10e-12) for p in preds]
            logloss = log_loss(labels, preds_safe)
            res["logloss"] = round(logloss, 4)
        elif metric == "acc":
            pred_binary = np.copy(preds)
