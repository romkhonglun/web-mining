...
class FastformerDataset(Dataset):
    ...
    def __post_init__(self):
        ...
        self.behaviors = self.behaviors.with_columns(pl.col(self.labels_col).list.len().alias(self.n_samples_col))
        self.lookup_indexes, self.lookup_matrix = make_lookup_objects(self.article_dict, unknown_representation="zeros")
    ...
    def __getitem__(self, index: int):
        ...
        # => article_ids_inview 数のリストを作る
        # (batch_size)
        repeats = np.array(batch[self.n_samples_col])
        # => history_input: article_id_history を、inview 数だけリピートする
        # (batch_size * num_inview, num_histories=30, num_tokens=40)
        history_input = repeat_by_list_values_from_matrix(
            input_array=x[self.history_column].to_list(),
            matrix=self.lookup_matrix,  # (num_articles=125542, num_tokens=40)
            repeats=repeats,
        ).squeeze(2)
        # => candidate_input: article_id_inview を、inview 数だけリピートする
        # (batch_size * num_inview, 1, num_tokens=40)
        ...
        y = torch.Tensor(batch[self.labels_col].explode()).view(-1, 1).type(torch.float).to(self.device)
        # ========================
...
def compute_auc_from_fixed_pos_neg_samples(y_true: list[float], y_pred: list[float]) -> float:
    #
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        if val_dataloader:
            ...
            if summary_writer is not None:
                summary_writer.add_scalar(tag="Val/Loss", scalar_value=val_loss, global_step=global_steps)
            ...
            if monitor_metric == "auc":
                ...
                if summary_writer is not None:
                    summary_writer.add_scalar(tag="Val/AUC", scalar_value=val_auc, global_step=global_steps)
