...
def compute_combinations(n: int, r: int) -> int:
    """Compute combinations (n choose r) where order does not matter (without replacement).

    Uses math.comb for exact integer results. Returns 0 when r > n.
    Raises:
        TypeError: if n or r are not integers.
        ValueError: if n or r are negative.
    """
